...
**Target hardware, stated once so it is never ambiguous.**

The goal is **all Intel GPUs that have XMX engines** — that is, the Xe-HPG generation (**Arc A-series**, Alchemist) **and** the Xe2 generation (**Arc B-series**, Battlemage), not a single part.

**The Intel Arc B580 (Xe2 / Battlemage) is the development and verification machine** — it is the one part physically present, so it is what can actually be measured, compiled against, and tested. It is **not** the sole target, and **its capabilities must never be generalised to the family**.

Three consequences follow, and they govern how every finding below must be read:

1. **The baseline is the intersection, not the B580.** Where a capability varies by generation, the **portable baseline is the set supported by *every* target generation**. A capability present only on B580 (or only on Xe2) is an **optional fast path gated by runtime detection**, never an assumption.
2. **Concretely, for XMX datatypes:** Xe-HPG is **confirmed** to support `FP16 / BF16 / INT8 / INT4 / INT2` and is **confirmed** to have **no TF32**. So **TF32 cannot be a baseline datatype for this project** regardless of whether the B580 supports it — a TF32 path may only exist as an **Xe2-and-later optional path behind a capability check**.
3. **A generation table is therefore not academic.** It is the mechanism by which the requirement is read off the **right row**, and by which "works on the B580" is prevented from being mistaken for "works on the family".

Wherever this document says "Xe" without qualification it means *the XMX-equipped family as a whole*. Wherever it says **B580** it means *the part available for measurement here*.
...
**Precision support — read as an intersection across target generations, not as one part's spec.**

> **Scope reminder.** The goal is **all XMX-equipped Intel GPUs**. The **B580 is only the machine available to test on**. A datatype present on B580 but absent on Xe-HPG is therefore **usable only as a detected optional path** — it cannot enter the baseline, because the baseline must run on **every** target.

| Generation | XMX data types | Meaning for this project |
|---|---|---|
| **Xe-HPG** (Arc A-series, Alchemist) | **FP16 / BF16 / INT8 / INT4 / INT2** — **no TF32** | **confirmed**; **these five are the portable baseline** |
| **Xe2** (Battlemage) — **includes the B580 (the test machine)** | **FP16 / BF16 / INT8 / INT4 / INT2**, with **TF32 added** | **reported, not independently confirmed**; if true it is an **Xe2+ optional path**, not a baseline |

**The portable datatype baseline for this project is therefore `FP16 / BF16 / INT8 / INT4 / INT2`.** That set is supported on both generations, so code restricted to it runs on **every** XMX target. Anything beyond it — **TF32 in particular** — is subject to the rules below.

**Rules for generation-dependent capabilities (this is the compatibility requirement, not a footnote).**

1. **No capability may be assumed from the existence of XMX engines.** "Has XMX" does **not** imply "has datatype *X*". Datatypes vary by generation.
2. **Everything outside the baseline must be detected at runtime**, not assumed at compile time or at port time. The device capability query path is the mechanism; a build-time assumption about a specific part is not acceptable.
3. **Every generation-dependent path needs a portable fallback** that uses only the baseline set. A TF32 fast path is only acceptable if the **FP16/BF16 path remains reachable** when TF32 is absent.
4. **Verification on the B580 proves the B580.** Because the B580 is Xe2, a B580 pass **says nothing** about Xe-HPG. Any claim of family-wide compatibility must come from either a capability query across generations or testing on an Xe-HPG part.

**What this means concretely for the B580 test machine.**
...
- **Even if TF32 is confirmed present on the B580, it stays an optional Xe2+ path** — because Xe-HPG is confirmed to lack it, and the project targets the family.
...
1. **There is no B580-specific datatype declaration behind the TF32 row — and a B580-specific answer would not settle the project question anyway.** The datatype statements available are: (a) a per-**product-family** table that covers **Xe-HPG (Arc A-Series)** and **lists Arc B-Series in its product set** without giving a B-Series datatype row; (b) a documentation sentence listing TF32 only as a type supported **"depending on hardware generation"**, without naming the generation; (c) a vendor **blog** post — *not* specification-grade evidence — listing TF32 among DPAS datatypes. **None of these is a statement that "the Arc B580 supports TF32 in XMX."** The source that would settle it is the vendor's **compiler-level `DPAS` instruction documentation** (`intel-graphics-compiler/documentation/visa/instructions/DPAS.md`), which **could not be retrieved from this environment**. **Porting decision that follows: TF32 is not a baseline datatype** — it may only exist as an **Xe2+ optional path behind a runtime capability check**. Separately, **no per-generation datatype table covering Xe-HPG vs Xe2 side by side was located**, which is what a family-wide compatibility claim would require.
2. **The "TF32 throughput is about half of BF16/FP16" remark is not confirmed.** No source read for this record states that ratio. It is marked **Low** confidence and should be treated as a lead, not a figure. The *direction* — that FP32-shaped GEMM is commonly run via BF16 decompositions (`bf16x2` / `bf16x3`) rather than a native FP32 path — **is** documented and is recorded above; **that path uses only baseline datatypes and is therefore family-wide relevant**.
...
5. **The family-wide target still needs a second physical data point.** Only an **Xe2 (B580)** part is available here. The Xe-HPG side of every generational claim rests on **vendor documentation**, not on a test machine. A claim of family-wide compatibility should be closed by a **capability query or a test run on an Xe-HPG part**.
...
|---|---|---|---|---|
| **G-01** | Does XMX provide directly callable matrix-multiply / convolution primitives, and what precision modes are covered? | The primitive is **DPAS** (dot-product-accumulate, `D = C + A×B`); **no direct convolution instruction**. Convolution / matmul are accelerated by **mapping onto DPAS** (e.g. oneDNN, default-on for Xe 4th-gen and later; or `joint_matrix_mad`). **Portable datatype baseline (all XMX generations): `FP16 / BF16 / INT8 / INT4 / INT2`** — Xe-HPG is **confirmed** to support exactly this set and **confirmed** to have **no TF32**, so TF32 can only be an **Xe2+ optional path behind runtime detection**. FP32-shaped GEMM has a **documented approximate path** via `bf16x2` / `bf16x3` (**uses baseline datatypes only**). Acceleration is **not automatic** — the datatype must be supported *on that generation* and the op must go through a mapping layer. Only compute-intensive, matmul-decomposable ops are candidates; elementwise / reduction / softmax / transpose are **not** XMX candidates. | **High** (primitive, mapping guidance, portable baseline); **Medium/Low** (Xe2/TF32 row and the TF32 throughput remark) | **No per-generation table covering Xe-HPG vs Xe2 side by side was located.** Xe-HPG's row is confirmed; **"TF32 on Xe2" stays reported** pending Intel's compiler-level `DPAS` instruction documentation, and **would not enter the baseline even if confirmed**. The "TF32 ≈ half BF16 throughput" figure is **unconfirmed** and marked Low. Family-wide claims still lack a second physical data point (only an Xe2 part is available here). |
